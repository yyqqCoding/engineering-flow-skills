...
多数工作流插件靠直觉堆规则，Engineering Flow 用数据裁决：每个触发方式和关键措辞都经过隔离的 A/B 基准检验——17 个场景的通用 cohort、独立的任务级多轮套件、隐藏评分器、污染检测和未授权提交监控。跑不赢强基线的功能会被删除，而不是保留下来装点门面：自动技能触发就是这样被否决的（数据反复证明它会在不相关任务上误载工作流）。每个决策的证据链都可以在 [基准记录](docs/benchmark-log.md) 中回溯。

最新任务级配对 A/B 在相同模型、推理等级和场景指纹下覆盖澄清批处理、Develop 连续性、需求文档生命周期和 Diagnose 连续性：当前发布版 control 为 `0/12`，最终 candidate 为 `12/12`。这证明的是这些隔离场景中的行为改进，不是对所有模型和项目的普遍保证。
...
实现订单批量导出。先理解现有设计和代码边界，澄清到足以安全实施后输出确认内容并暂停；我批准后再完成实现、聚焦测试和必要文档更新。不要提交。
```
...
同一任务的回答、批准、纠正和原验收遗漏会自动延续该工作流，不必重复输入 token。
