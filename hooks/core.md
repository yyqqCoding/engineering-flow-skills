...
- For new or changed behavior, complete the production implementation before adding or editing tests. Then add only coverage that protects critical behavior or established risk boundaries; otherwise use the smallest meaningful build, type, lint, integration, smoke, visual, or behavioral check. For a regression with a stable seam, observe a focused failing test before the fix.
