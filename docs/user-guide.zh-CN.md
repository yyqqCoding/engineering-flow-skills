...
按你需要的结果选择入口，各工作流独立负责完成当前任务。Develop 自身包含实施所需的设计、诊断修复、自检和交付。

| 场景 | 工作流 | 是否修改代码 |
|---|---|---|
| 新功能、重构、补测试、代码完善 | `develop` | 检查点获明确批准后修改 |
| bug、回归、错误输出、间歇故障、性能下降 | `diagnose` | 用户要求修复时修改 |
| 从 0 出方案或完善已有设计 | `code-design` | 否 |
| 评审 diff、分支或未提交改动 | `review` | 评审阶段只读；明确要求后修复指定问题 |
| 保存任务状态与授权，交给新会话或其他 agent | `handoff` | 否 |
...
2. 对齐目标、具体验收实例、范围、事实和重要方案决定。
3. 批量询问相互独立的重要问题，按依赖顺序询问后续问题，并在足以安全实施时停止。
4. 输出最终确认内容并暂停，等待明确的实施批准。
5. 批准后按完整、可观察的行为逐段实施与验证，由风险决定测试时机。
6. 在同一任务内完成实施所需的设计、诊断修复和自检。
7. 用新鲜证据核对已确认的验收实例，并同步发生变化的权威文档。
...
验收实例把需求与验证连起来。例如，已确认的行为是“按创建时间倒序导出订单”，实例就应给出两笔时间不同的订单，并说明新订单在前。只检查“生成了一个文件”无法发现对排序的理解偏差。

实施前，模型从已确认的行为和仓库契约推导预期结果；实施中，每完成一段有意义的行为就验证它。关键断言可以提前固定；有稳定测试入口的回归仍先观察失败再修复。机械或展示改动可以采用构建、类型、冒烟或视觉检查。纯测试任务验证覆盖的敏感性，无须制造生产代码改动。

每段实施与验证在现有批准范围内继续。针对已有风险补充测试属于正常验证；期望业务行为或批准范围变化时才回到对齐。完成前还会核对整体验收和实际交互，并说明仍缺少证据的项目。
...
上面的请求已包含修复授权，Diagnose 会直接完成复现、根因分析、修复、回归验证和交付。只要求诊断时保持只读，后续说“可以修复了”等明确指令后继续修复。用户否定诊断时，同一任务回到只读调查。整个过程无须另行调用 Develop。有稳定测试入口时，先观察失败再修复；未定义的产品行为或实质新增范围仍需对齐。
...
它默认只返回方案，不实现生产代码，也不会静默修改设计文档。如果你选择用 Develop 实施，已确定的目标、边界、决定和实例会被复用，仅针对缺项、变化或仓库证据矛盾补充对齐，再给出 Develop 的实施检查点。接受设计方案本身不等于批准实施。
...
评审会检查需求、正确性、安全、设计、可读性、测试、文档和范围。发现问题不会自动获得修复权限。后续明确要求“修复第 1、3 项”会授权在同一任务中核验这些发现、完成必要修复并重新验证，仍须满足原任务已有的批准检查点，无须再次调用 Develop；请求之外的发现保持不改。

### `handoff`

`handoff` 为新会话或其他 agent 保存最小必要状态，包括当前工作流与阶段、已确认行为、已有授权和仍待批准的事项。

```text
$engineering-flow:handoff
生成当前任务的续接记录，包含目标、当前工作流与阶段、已有授权、已完成状态、关键文件、决定、最新验证、剩余任务、风险和 Git 状态。
```

未指定输出路径时，交接内容只返回在回复中，不会静默创建文件。记录传递任务状态，不产生新授权；接收会话会结合当前仓库核对状态，继续已获授权的阶段，尚待批准的事项仍保持待批准。
...
以下为 v1.0.3 及更早 cohort 的历史记录，保留当时的 fixture、评分规则和插件指纹。它们不代表当前工作流与测试策略调整已经验证，本次调整需要独立证据记录。

- 静态与确定性测试：84/84 通过。
- 当时记录的语料包含 37 个已配置场景，在语义上覆盖全部 46 个行为 ID；这不等同于完成了 37 次模型试验。
- 已发布的 Codex 通用 cohort 为 17 个场景、candidate 51/51 通过；显式调用 51/51，误触发、漏触发、碰撞、污染和未授权提交均为 0。
...
- Claude Code 2.1.223 通过 strict manifest 校验；在当时的测试策略下，最终隔离的 `/engineering-flow:develop` 轨迹先写生产代码，再补关键边界测试。
- 历史 Claude 普通 Core-only 歧义样本未达到 Codex 同等行为，因此涉及数据、权限等重大决定时应显式调用完整工作流。
